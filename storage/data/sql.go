...
func (d *SQLDatabase) Init() error {
	...
	case MySQL:
		...
		// }
		type Items struct {
			ItemId     string    `gorm:"column:tid;type:int; not null;primaryKey"`
			IsHidden   int       `gorm:"column:is_hidden;type:bool;defautlt:false"`
			Categories []string  `gorm:"column:categories;type:json"`
			Timestamp  time.Time `gorm:"column:update_time;type:datetime"`
			Labels     []string  `gorm:"column:ty_know_id;type:json"`
			Comment    string    `gorm:"column:body;type:text"`
		}
		type Users struct {
...
func (d *SQLDatabase) GetItem(itemId string) (Item, error) {
	var result *sql.Rows
	var err error
	result, err = d.gormDB.Table("items").Select("tid, is_hidden, categories, update_time, ty_know_id, body").Where("tid = ?", itemId).Rows()
	// Limit(1).
	if err != nil {
...
func (d *SQLDatabase) GetItems(cursor string, n int, timeLimit *time.Time) (string, []Item, error) {
	...
	tx := d.gormDB.Table("items").Select("tid, is_hidden, categories, update_time, ty_know_id, body").Where("tid >= ?", cursor)

	if timeLimit != nil {
		tx.Where("time_stamp >= ?", *timeLimit)
	}
	// result, err := tx.Order("item_id").Limit(n + 1).Rows()
	result, err := tx.Order("tid").Limit(n + 1).Rows()
	if err != nil {
		return "", nil, errors.Trace(err)
	}
	items := make([]Item, 0)
...
func (d *SQLDatabase) GetItemStream(batchSize int, timeLimit *time.Time) (chan []Item, chan error) {
	...
	go func() {
		...
		defer close(errChan)
		// send query // change
		tx := d.gormDB.Table("items").Select("tid, is_hidden, categories, update_time, ty_know_id, body")
		if timeLimit != nil {
			tx.Where("time_stamp >= ?", *timeLimit)
		}
		result, err := tx.Rows()

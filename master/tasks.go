...
func (m *Master) runRankingRelatedTasks(
	...
	//debug
	// log.Logger().Info("test836 find better ranking model",
	...
	//debug
	log.Logger().Info("test859 find better ranking model",
		zap.Any("params", rankingModel.GetParams()))
	...
	if numItems == 0 {
		...
		log.Logger().Info("Training item model!")
		m.runFindItemNeighborsTask(m.rankingTrainSet)
		log.Logger().Info("Training  finish!")
	}
